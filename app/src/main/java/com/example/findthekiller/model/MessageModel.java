...
public class MessageModel {
    ...
    private void playerResponse()
    {
        ...
        switch (selectResponse) {
            case 0:
                message.append("At the time of the incident, I was ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(".");
                break;
            case 1:
                message.append(firstWordCasing(playerModel.getActivity())).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(". That's where I was when everything went down.");
                break;
            case 2:
                message.append("During the incident, my focus was ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(".");
                break;
            case 3:
                message.append("I was right in the middle of ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(" when the incident took place.");
                break;
            case 4:
            ...
            case 6:
                ...
                break;
            case 7:
                message.append("In the moments of the incident, my focus was ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(" creating a vibrant atmosphere.");
                break;
            case 8:
                message.append(firstWordCasing(playerModel.getActivity())).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(". The incident became an unexpected twist in the activity, adding an unexpected layer to the evening.");
                break;
            case 9:
                message.append("Amidst ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(". The incident cast an unexpected shadow over the good moment.");
                break;
            case 10:
                message.append("In the midst of ").append(playerModel.getActivity()).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(". The incident injected an unexpected twist into the night.");
                break;
            case 11:
                message.append(firstWordCasing(playerModel.getActivity())).append(groupChat).append(" at the ").append(playerModel.getRoom()).append(". Little did I know that the incident would become a pivotal point in the otherwise carefree evening.");
                break;
        }
    ...
    private String firstWordCasing(String sentence)
    {
        String firstString = "", remainingString = "";

        if(!sentence.isEmpty())
        {
            firstString = sentence.substring(0, 1).toUpperCase();
            remainingString = sentence.substring(1);
        }
